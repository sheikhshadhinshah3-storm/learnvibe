...
export default function DarkHome() {
  ...
  return (
    <div className="relative">
      {/* Hero — left-aligned, elegant dark */}
      <section className="mx-auto max-w-5xl px-4 pb-16 pt-16 sm:px-6 sm:pb-24 sm:pt-24 lg:pt-32">
        <FadeContent blur duration={800} delay={100}>
          <div className="min-w-0 max-w-3xl">
            <div className="mb-8 inline-flex max-w-full items-center gap-2 rounded-full border border-emerald-500/20 px-3 py-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span className="min-w-0 break-words text-xs font-medium tracking-wide text-emerald-400/80">{homeContent.heroTagline}</span>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:mt-10 sm:flex-row sm:items-center sm:gap-4">
              {user ? (
                <Link to="/dashboard" className="px-7 py-3 rounded-lg bg-emerald-500 text-black font-semibold text-sm hover:bg-emerald-400 transition-colors">
          ...
          <FadeContent blur duration={800} delay={100}>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {enabledModels.slice(0, 8).map((m, i) => (
                <div key={m.id || i} className="px-4 py-3 rounded-lg border border-neutral-800/60 hover:border-emerald-500/20 transition-colors group">
                  <span className="text-sm text-neutral-400 font-mono group-hover:text-neutral-300 transition-colors truncate block">{m.display_name || m.model_name}</span>
                </div>
              ))}
            </div>
          </FadeContent>
            ...
            <div className="grid md:grid-cols-3 gap-4 max-w-4xl">
              {packages.filter(p => p.enabled).slice(0, 3).map((pkg, i) => {
                ...
                <div key={pkg.id} className={`rounded-xl p-6 flex flex-col border transition-colors ${
                  i === 1 ? 'border-emerald-500/25 bg-emerald-500/[0.04]' : 'border-neutral-800/60 hover:border-neutral-700'
                }`}>
                  {i === 1 && <span className="text-xs text-emerald-400 font-medium mb-2 uppercase tracking-wider">{t('home.popular') || 'Popular'}</span>}
